    ...
    <div class="login-card admin-panel">
        ...
        @if($section === 'seo')
            ...
            <div style="margin-top:14px;padding:12px;border:1px solid #e2e8f0;border-radius:10px;background:#f8fafc;">
                ...
                <p class="hint-text" style="margin-top:4px;">PNG, JPG, GIF, WebP, SVG o ICO. Recomendado <strong>32×32</strong> o <strong>48×48</strong> px (ICO puede incluir varias resoluciones). Máx. ~1,5 MB. Google Search Console usa este icono en resultados.</p>
                @if($favSrc !== '')
...
RABBITMQ_ADMIN_QUEUE_NAMES=media,default</pre>
  ...
  rabbitmq:3-management</pre>
        ...
        @if($section === 'monitoreo')
            ...
            <div id="admin-monitoreo-storage-root" class="mt-2" data-storage-audit-url="{{ route('admin.monitoreo.storage_audit') }}">
                <p class="hint-text text-slate-600" id="admin-monitoreo-storage-status">Cargando análisis de almacenamiento… (en volúmenes grandes puede tardar varios segundos; la página ya está lista).</p>
            </div>
            <script>
                (function () {
                    var root = document.getElementById('admin-monitoreo-storage-root');
                    if (!root || !root.dataset.storageAuditUrl) return;
                    var url = root.dataset.storageAuditUrl;
                    function load() {
                        var status = document.getElementById('admin-monitoreo-storage-status');
                        if (status) {
                            status.textContent = 'Cargando análisis de almacenamiento…';
                        }
                        fetch(url, {
                            credentials: 'same-origin',
                            ...
                            .catch(function () {
                                root.innerHTML =
                                    '<div class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900" style="margin:12px 0;">No se pudo cargar el análisis de almacenamiento. <button type="button" class="btn-secondary" id="admin-monitoreo-storage-retry">Reintentar</button></div>';
                                var btn = document.getElementById('admin-monitoreo-storage-retry');
